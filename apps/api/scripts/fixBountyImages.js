// 一次性脚本：Bing 检索 + 标题关键词过滤，重下悬赏对应图片
// 用法：node scripts/fixBountyImages.js
// 说明：下载覆盖 bounty-01~12.jpg；bounty-10（日本寺庙）保留原图。
import fs from "fs";
...
// 每条悬赏：检索词 + 标题必须包含的关键词；skip=true 保留原图
const BOUNTIES = [
  { file: "bounty-05.jpg", query: "鸣沙山月牙泉", titleNeed: /月牙泉|鸣沙山/ },
  { file: "bounty-07.jpg", query: "武汉大学樱花城堡", titleNeed: /武大|武汉大学|樱花/ }
];
...
(async () => {
  for (const b of BOUNTIES) {
    if (b.skip) { console.log(`SKIP ${b.file}（保留原图）`); continue; }
    let done = false;
    try {
      const all = await search(b.query);
      let cands = all.filter((c) => b.titleNeed.test(c.title));
      console.log(`— ${b.file} (${b.query}) 候选 ${all.length}，标题命中 ${cands.length}`);
      ...
      for (const c of cands.slice(0, 20)) {
        try {
          ...
          fs.writeFileSync(path.join(dir, b.file), buf);
          console.log(`  OK ${b.file} ${size.width}x${size.height} ${(buf.length / 1024).toFixed(0)}KB\n     标题:${(c.title || "").slice(0, 50)}`);
          done = true;
          break;
        } catch (e) { /* 下一张 */ }
      }
      if (!done) console.log(`  FAIL ${b.file}`);
    } catch (e) {
      console.log(`  FAIL ${b.file} (${e.message})`);
    }
    await sleep(800);
  }
  console.log("\n完成。");
